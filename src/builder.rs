    ...
    MODEL::ColorFormat: InterfacePixelFormat<DI::Word>,
{
    ...
    ///
    #[must_use]
    pub fn new_async(model: MODEL, di: DI) -> Self {
        Self {
            di,
            model,
            rst: None,
...
impl<DI, MODEL> Builder<DI, MODEL, NoResetPin> {
    ...
    ///
    #[must_use]
    pub fn reset_pin<RST>(self, rst: RST) -> Builder<DI, MODEL, RST>
    where
        RST: OutputPin,
    {
        Builder {
    ...
    RST: OutputPin,
{
    ...
    pub fn init(
        ...
        let mut ie = InitEngineSync::new(self.di, delay_source);
...
{
    /// TODO
    pub async fn init_async<'buffer>(
        mut self,
        delay_source: &mut impl AsyncDelayNs,
        buffer: &'buffer mut [u8],
    ) -> Result<DisplayAsync<'buffer, DI, MODEL, RST>, InitError<DI::Error, RST::Error>> {
        let to_u32 = |(a, b)| (u32::from(a), u32::from(b));
        let (width, height) = to_u32(self.options.display_size);
        let (offset_x, offset_y) = to_u32(self.options.display_offset);
        let (max_width, max_height) = to_u32(MODEL::FRAMEBUFFER_SIZE);
        ...
        let mut ie = InitEngineAsync::new(&self.di);
